...
int Wave3d::eval(ParVec<int,cpx,PtPrtn>& den, ParVec<int,cpx,PtPrtn>& val)
{
    ...
    for(map<BoxKey,BoxDat>::iterator mi = _boxvec.lclmap().begin();
        ...
        if (ispts(curdat) && own_box(curkey, mpirank) && isterminal(curdat)) {
    ...
    int DPG = 4;
    int NG = (TTL-1) / DPG + 1;
    // TODO (Austin): Can we remove this if(1) ?
    if(1) {
        int cur = 0;
    ...
    t1 = time(0);
    if(mpirank==0) {
        cout<<"LOW DN "<<difftime(t1,t0)<<"secs "<<endl;
    }
    iC( MPI_Barrier(MPI_COMM_WORLD) );
    //set val from extval
    vector<int> wrtpts;
    ...
    for(map<BoxKey,BoxDat>::iterator mi = _boxvec.lclmap().begin();
        ...
        BoxDat& curdat = mi->second;
        if(ispts(curdat)) {
            if(own_box(curkey, mpirank) && isterminal(curdat)==true) {
